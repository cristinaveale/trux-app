...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    updateKudo({ commit, state }, repo) {
      ...
      return APIClient.updateKudo(repo).then(() => {
        commit("resetKudos", kudos);
      });
    },
    toggleKudo({ commit, state }, repo) {
      if (!state.kudos[repo.id]) {
        return APIClient.createKudo(repo).then(kudo =>
          commit("resetKudos", { [kudo.id]: kudo, ...state.kudos })
        );
      }

      const kudos = Object.entries(state.kudos).reduce(
        (acc, [repoId, kudo]) => {
          return repoId == repo.id ? acc : { [repoId]: kudo, ...acc };
        },
        {}
      );

      return APIClient.deleteKudo(repo).then(() => commit("resetKudos", kudos));
    }
  }
});
